...
export default function OwnerChatScreen() {
  ...
  const handleDelete = (id: string) => {
    showAlert(
      'Hapus Chat',
      'Apakah Anda yakin ingin menghapus percakapan ini dari riwayat Anda?',
      'warning',
      async () => {
  ...
  const renderItem = ({ item }: { item: Conversation }) => (
    <TouchableOpacity 
      style={styles.card} 
      onPress={() => router.push({
        pathname: '/(owner)/chat/[id]',
        params: { id: item.otherUserId, name: item.otherUserName || 'Pencari Kost', role: item.otherUserRole }
      })}
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Kelola pertanyaan dari calon penyewa</Text>
      </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <View style={styles.emptyIconCircle}>
                <FontAwesome5 name="comment-slash" size={40} color="#CBD5E1" />
              </View>
              <Text style={styles.emptyText}>Belum Ada Pesan</Text>
              <Text style={styles.emptySubtext}>Pesan dari calon penyewa akan muncul di sini.</Text>
            </View>
          }
        />
      )}
